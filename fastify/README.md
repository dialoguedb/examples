...
A REST API for AI-powered conversations with persistent history. [Fastify](https://fastify.dev) handles routing and request validation, [DialogueDB](https://dialoguedb.com) stores the conversation history, and Claude provides the AI responses.

**Why this matters:** Most chat backends use in-memory storage or a hand-rolled database schema. DialogueDB gives you persistent, queryable conversation storage with zero schema setup — so Fastify handles HTTP and DialogueDB handles state.

> **Also see:** [`../hono/`](../hono/) for a Hono-based server, [`../anthropic-sdk/`](../anthropic-sdk/) for direct Claude API integration.
