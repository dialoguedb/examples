...
/** Send a message array to Llama, return the text response. */
async function chat(
...
async function main() {
  console.log("=== DialogueDB + Together AI: Hello World ===\n");

  // 1. Create a new conversation
  const dialogue = await db.createDialogue({
    label: "together-hello-world",
    state: { provider: "together", format: "openai-chat", model: MODEL },
  });
  ...
  await dialogue.saveMessage({
    role: "user",
    content:
      "Hi! My name is Jordan and I'm building a fitness tracking app that uses AI to generate personalized workout plans based on user goals and equipment availability. What architecture would you suggest?",
  });
  ...
  await dialogue.saveMessage({
    role: "user",
    content:
      "Great suggestions. I also want to support progress photos with AI form analysis and a social feed where users can share workouts. How would you extend the architecture?",
  });
