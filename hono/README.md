...
A lightweight REST API for AI-powered conversations with persistent history. [Hono](https://hono.dev) handles routing, [DialogueDB](https://dialoguedb.com) stores the conversation history, and Claude provides the AI responses.

**Why this matters:** Most chat backends use in-memory storage or a hand-rolled database schema. DialogueDB gives you persistent, queryable conversation storage with zero schema setup — so Hono handles HTTP and DialogueDB handles state.

> **Also see:** [`../anthropic-sdk/`](../anthropic-sdk/) for direct Claude API integration, [`../anthropic-agent-sdk/`](../anthropic-agent-sdk/) for autonomous agent patterns.
...
Runs both server instances in-process to demonstrate persistence across cold restarts:
...
3. Simulates a cold restart (new app instance, no in-memory state)
